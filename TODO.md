...
## Search Enhancements
- [ ] `find_closest(point, max_dist, norm)`: Return nearest neighbor only if within `max_dist`. (Diff: 2/5)
- [ ] `find_all_within(point, max_dist, norm)`: Return all entries within a radius. (Diff: 3/5)
- [ ] `find_closest_k(point, k, norm)`: Find the K nearest neighbors. (Diff: 3/5)
